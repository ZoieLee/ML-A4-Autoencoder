...
def scatter_plot(latent_representations, labels):
    ...
    mapper = umap.UMAP()
    latent_representations = mapper.fit_transform(latent_representations)
...
class VAE_Trainer(object):
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, _) in tqdm(enumerate(self.train_loader), total=len(self.train_loader) ) :
            ...
            recon_batch, code_mu, code_logvar = self.model(data)
            loss = self.loss_function(recon_batch, data, code_mu, code_logvar)
            loss.backward()
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, (data, _) in tqdm(enumerate(self.val_loader), total=len(self.val_loader)):
                data = data.to(self.device)
                recon_batch, code_mu, code_logvar = self.model(data)
                val_loss += self.loss_function(recon_batch, data, code_mu, code_logvar).item()
